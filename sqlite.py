...
def save_deidentified_instance_db(bytes, instance_id):
    ...
    try:
        ...
        cursor.execute("INSERT INTO deidentified VALUES(?, ?, ?, ?) ON CONFLICT DO NOTHING", 
...
def save_secured_instance_db(bytes, instance_id):
    conn = sqlite3.connect("dicom.db")
    cursor = conn.cursor()
    ...
    try:
        ...
        cursor.execute("INSERT INTO secure VALUES(?, ?, ?, ?) ON CONFLICT DO NOTHING", 
                    (instance_id, bytes, datetime.datetime.now().isoformat(), 'instance'))
    except Exception as e:
        print('Exception: {}'.format(e))
        raise Exception(e)
...
def save_signed_instance_db(bytes, instance_id, type):
    conn = sqlite3.connect("dicom.db")
    cursor = conn.cursor()
    ...
    try:
        ...
        cursor.execute("INSERT INTO signed VALUES(?, ?, ?, ?) ON CONFLICT DO NOTHING", 
                    (instance_id, bytes, datetime.datetime.now().isoformat(), type))
    except Exception as e:
        print('Exception: {}'.format(e))
        raise Exception(e)
    finally:
